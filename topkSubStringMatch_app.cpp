...
int main(int argc, const char* argv[])
{
	...
	ofstream fout((stemp + "_subString_app").c_str());
	fout << "K,  Average_max_ed, Average_candidate_number, Average_indextime, Average_initialtime, Average_acctime, Average_querytime, Average_time" << endl;
	...
	for (int j = 0; j < tk.size(); j++) {
		...
		ofstream fdetail((stemp + "_subString_app_" + changeI2S(gl_topk)).c_str());
		...
		for (unsigned i = 0; i < queryset.size(); i++) {//get each query
			...
			db_load.subString_initial_approximate(query);
			_time2.check();
	...
	fout << gl_topk << "  " << max_ed / queryset.size() << "  " << processednum / queryset.size() << "  " 
		 ...
		 << query_time / queryset.size() << "  " << all_time / queryset.size() << endl;
	}
