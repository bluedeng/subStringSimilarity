...
void usage()
{
	...
	fprintf(stdout, "----------------------------------------------\n\n");
	fprintf(stdout, "Usage: topkSubStringMatching [OPTION] ${INPUTDB} ${QUERY}\n");
...
int pipe_knn(CQuery* query) {
	...
	while (true)
	{
		if (threadnum <= 0)
		{
			...
			pThis->subString_process();
			class TSINGHUA_CLIPSE_UTIL::TimeRecorder _time2;
			pThis->tail_process();
			_time2.check();
			tail_time += _time2.diffTime(0, 1);
			_time1.check();
...
int main(int argc, const char* argv[])
{
	...
	ofstream fout((stemp + "_subString_exa").c_str());
	fout << "K,  Average_max_ed, Average_candidate_number, Average_indextime, Average_initialtime, Average_acctime, Average_querytime, Average_tailtime, Average_time" << endl;
	...
	for (int j = 0; j < tk.size(); j++) {
		...
		ofstream fdetail((stemp + "_subString_exa_" + changeI2S(gl_topk)).c_str());
		...
		for (unsigned i = 0; i < queryset.size(); i++) {//get each query
			...
			db_load.subString_initial_exact(query);
			_time2.check();
			init_time += _time2.diffTime(0, 1);
		...
		fout << gl_topk << "  " << max_ed / queryset.size() << "  " << processednum / queryset.size() << "  "
			<< index_time << "  " << init_time / queryset.size() << "  " << acc_time / queryset.size() << "  "
			<< query_time / queryset.size() << "  " << tail_time / queryset.size() << "  " << all_time / queryset.size() << endl;
	}
